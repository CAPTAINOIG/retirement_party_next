      ...
      {view === 'options' && (
        <>
          ...
          <div className="px-10 md:px-16 -mt-14 pb-12 space-y-4 relative z-[2]">
            <Card hover onClick={() => setView('chat')} className="px-7 py-5 flex items-center">
              <IconMessageChatbot size="32" className="text-blue-600" />
              <div className="flex-1 px-4">
                <p>Chat with Me</p>
                <p className="text-[.94rem] mt-1 opacity-70 leading-snug">Ask questions about macro economic data.</p>
              </div>
              <IconChevronRight className="opacity-70" />
            </Card>
            <Card hover onClick={() => setView('document')} className="px-7 py-5 flex items-center">
              <IconFileTextAi size="32" className="text-orange-600" />
              <div className="flex-1 px-4">
                <p>Analyze docs</p>
                <p className="text-[.94rem] mt-1 opacity-70 leading-snug">Analyze and chat with your documents.</p>
              </div>
              <IconChevronRight className="opacity-70" />
            </Card>
          </div>
          ...
          {!isChatEnabled ? (
            <div className="h-full flex flex-col overflow-hidden">
              ...
              <div className="flex flex-col items-center justify-center text-center my-auto">
                ...
                <div className="px-2.5 py-1 rounded-full text-xs bg-red-500 text-white w-max mt-8">Coming soon</div>
              </div>
            </div>
          ) : (
            <Chat onBack={() => setView('options')} />
          )}
        </>
      )}
      {view === 'document' && (
        <>
          {!isChatEnabled ? (
            ...
            <UploadDocument onBack={() => setView('options')} />
          )}
        </>
      )}
    </Drawer>
