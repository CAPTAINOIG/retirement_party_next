...
const InfographicCard = ({ infographic }) => {
  return (
    <div className="w-full overflow-hidden">
      <Link href={ `/infographics/${ infographic.category.id }/${ infographic.id }` } className="flex w-full">
        <Image src={ infographic.image } alt={ infographic.title } className="rounded-xl h-60 w-full object-cover"/>
      </Link>
      <div className="mt-4 overflow-hidden">
        <p className="opacity-90">{ formatDistance(new Date(), new Date(infographic.created_at)) }</p>
        <Link href={ `/infographics/${ infographic.category.id }/${ infographic.id }` }>
