...
import { Button, Input, Drawer, DrawerBody, DrawerContent, addToast } from '@heroui/react';
import { useCreateEventAttendee } from '@/api/other';
import { TbSend, TbUser, TbMail, TbPhone, TbSchool, TbCheck, TbX } from 'react-icons/tb';

const CampusRegistrationDrawer = ({ isOpen, onClose }) => {
  ...
  const onSubmit = async (data) => {
    try {
      await createAttendee({
        ...
        email: data.email,
        phone: data.phone,
        event: 'campus-data-training-program',
        meta: {
          school: data.school,
        },
      });
  ...
  const SuccessView = () => (
    <div className="py-12 text-center">
      ...
      >
        Thank you for registering for the Campus Data Training Program! We're excited to help you build real data
        skills.
      </motion.p>
      ...
      >
        ...
        <ul className="space-y-2 text-sm opacity-80">
          ...
          <li className="flex items-start">
            <span className="bg-primary-500 mt-1 mr-2 h-1.5 w-1.5 rounded-full"></span>
            We'll contact you via WhatsApp with registration dates
          </li>
          <li className="flex items-start">
            <span className="bg-primary-500 mt-1 mr-2 h-1.5 w-1.5 rounded-full"></span>
            Get ready to build skills that actually increase employability
          </li>
        </ul>
      </motion.div>
      ...
      <DrawerContent>
        <DrawerBody className="p-6 md:p-8">
          <div className="mb-6 flex items-center justify-between gap-4">
            <div>
              <h3 className="text-2xl font-bold">Register for Campus Program</h3>
              <p className="mt-1 text-base opacity-80">Join the next generation of data thinkers across Africa</p>
            </div>
...
export default memo(CampusRegistrationDrawer);
